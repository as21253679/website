  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <?php
		$id=$_GET["id"];
		echo "<form action='update_farm_a.php?id=$id' method='POST'>";
		?>
...
<?php  
			...
			$conn = mysql_connect($dbhost, $dbuser, $dbpass) ;//連接資料庫
			 ...
			 $sql = "select * from farm where farmID=$id";//查詢整個表單
			  $result = mysql_query($sql);
						
			while($row = mysql_fetch_array($result)){//印出資料
			 echo "<div class='form-group'>
                <label for='back-Creat-Header'>小農姓名</label>
                <input type='text' class='form-control' id='back-Creat-Header' placeholder='請填寫姓名' name='name' value='".$row['name']."'>
              </div>";
              echo "<div class='form-group'>
                <label for='back-Creat-content'>產品介紹</label>
                <textarea class='form-control' rows='15' id='back-Creat-content' placeholder='懇請提供產品相關資訊以及生產時間等相關資訊' name='infor'>".$row['product']."</textarea>
              </div>";
			  mysql_close($conn);
			}
?>
